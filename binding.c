...
static WBindmap bindmaps[ACT_N] = {{0, NULL}, {0, NULL}, {0, NULL}, {0, NULL}, {0, NULL}};
...
static const uint modmasks[N_MODS] = {ShiftMask, LockMask, ControlMask, Mod1Mask,
...
static void evil_grab_key(Display *display, uint keycode, uint modifiers, Window grab_window,
                          bool owner_events, int pointer_mode, int keyboard_mode);

static void evil_grab_button(Display *display, uint button, uint modifiers, Window grab_window,
                             bool owner_events, uint event_mask, int pointer_mode,
                             int keyboard_mode, Window confine_to, Cursor cursor);

#endif

bool add_binding(uint act, uint actx, uint state, uint kcb, const WFuncBinder *binder) {
  WBinding *binding;
  WBindmap *bindmap;
  int i;

  if (binder->func == NULL || act >= ACT_N) return FALSE;

  bindmap = &(bindmaps[act]);

  binding = REALLOC_N(bindmap->bindings, WBinding, bindmap->nbindings, bindmap->nbindings + 1);
  ...
  if (act == ACT_KEYPRESS) kcb = XKeysymToKeycode(wglobal.dpy, kcb);

  for (i = 0; i < bindmap->nbindings; i++) {
    if (binding[i].kcb >= kcb) break;
  }

  for (; i < bindmap->nbindings; i++) {
    if (binding[i].kcb > kcb) break;
    if (binding[i].state > state) break;
  }

  memmove(&(binding[i + 1]), &(binding[i]), sizeof(WBinding) * (bindmap->nbindings - i));
...
bool add_pointer_binding(uint actx, uint state, uint button, const WFuncBinder *pressb,
                         const WFuncBinder *clickb, const WFuncBinder *dblclickb,
...
static void do_grab_keys(WBindmap *bindmap, Window win, uint actx) {
  ...
  for (i = 0; i < bindmap->nbindings; i++, binding++) {
    if (!(binding->actx & actx)) continue;

#ifndef CF_HACK_IGNORE_EVIL_LOCKS
    XGrabKey(wglobal.dpy, binding->kcb, binding->state, win, True, GrabModeAsync, GrabModeAsync);
#else
    evil_grab_key(wglobal.dpy, binding->kcb, binding->state, win, True, GrabModeAsync,
                  GrabModeAsync);
#endif
  }
}
...
static void do_grab_buttons(WBindmap *bindmap, Window win, uint actx) {
  ...
  for (i = 0; i < bindmap->nbindings; i++, binding++) {
    ...
    if (binding->state == 0) continue;

#ifndef CF_HACK_IGNORE_EVIL_LOCKS
    XGrabButton(wglobal.dpy, binding->kcb, binding->state, win, True, GRAB_POINTER_MASK,
                GrabModeAsync, GrabModeAsync, None, None);
#else
    evil_grab_button(wglobal.dpy, binding->kcb, binding->state, win, True, GRAB_POINTER_MASK,
                     GrabModeAsync, GrabModeAsync, None, None);
#endif
  }
}
...
void grab_bindings(Window win, uint actx) {
  ...
  for (i = 1; i < ACT_N; i++) do_grab_buttons(&(bindmaps[i]), win, actx);
}
...
again:
  ...
  for (i = 0; i < bindmap->nbindings; binding++, i++) {
    if (binding->kcb >= kcb) break;
  }

  for (; i < bindmap->nbindings; binding++, i++) {
    if (binding->kcb > kcb) break;
    if (binding->state > state) break;
    if (binding->state < state) continue;
    if (binding->actx & actx) return binding;
  }

  if (act == ACT_KEYPRESS) {
    if (kcb == AnyKey) return NULL;
    kcb = AnyKey;
  } else {
    if (kcb == AnyButton) return NULL;
    kcb = AnyButton;
  }
...
static void lookup_evil_locks() {
  ...
  for (i = 0; i < N_LOOKUPEVIL; i++) keycodes[i] = XKeysymToKeycode(wglobal.dpy, evillocks[i]);
...
static void evil_grab_key(Display *display, uint keycode, uint modifiers, Window grab_window,
  ...
  for (i = 0; i < N_EVILLOCKS; i++) {
    ...
    for (j = i; j < N_EVILLOCKS; j++) {
      ...
      XGrabKey(display, keycode, mods, grab_window, owner_events, pointer_mode, keyboard_mode);
      if (i == j) continue;
      XGrabKey(display, keycode, modifiers | evillockmasks[i] | evillockmasks[j], grab_window,
...
static void evil_grab_button(Display *display, uint button, uint modifiers, Window grab_window,
  ...
  XGrabButton(display, button, modifiers, grab_window, owner_events, event_mask, pointer_mode,
  ...
  for (i = 0; i < N_EVILLOCKS; i++) {
    ...
    for (j = i; j < N_EVILLOCKS; j++) {
      if (evillockmasks[j] == 0) continue;
      mods |= evillockmasks[j];
      XGrabButton(display, button, mods, grab_window, owner_events, event_mask, pointer_mode,
                  keyboard_mode, confine_to, cursor);
      if (i == j) continue;
      XGrabButton(display, button, modifiers | evillockmasks[i] | evillockmasks[j], grab_window,
