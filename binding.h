...
extern bool add_binding(uint act, uint actx, uint state, uint kcb, const WFuncBinder *binder);
extern void grab_bindings(Window win, uint actx);
extern WBinding *lookup_binding(uint act, uint actx, uint state, uint kcb);

extern bool add_pointer_binding(uint actx, uint state, uint kcb, const WFuncBinder *pressb,
