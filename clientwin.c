...
void get_clientwin_size_hints(WClientWin *cwin) {
  ...
  if (hints->flags & PMaxSize) {
    if (hints->max_width < hints->base_width) hints->max_width = hints->base_width;
    if (hints->max_height < hints->base_height) hints->max_height = hints->base_height;
  }
  ...
  if (hints->flags & PAspect) {
    if (hints->min_aspect.x <= 0 || hints->min_aspect.y <= 0 || hints->min_aspect.x <= 0 ||
  ...
  if (CWIN_HAS_FRAME(cwin)) frame_recalc_minmax(CWIN_FRAME(cwin));
}

static void limit_size(WClientWin *cwin, int *w, int *h) {
  if (cwin->size_hints.flags & PMinSize) {
    if (*w < cwin->size_hints.min_width) *w = cwin->size_hints.min_width;
    if (*h < cwin->size_hints.min_height) *h = cwin->size_hints.min_height;
  }

  if (cwin->size_hints.flags & PMaxSize) {
    if (*w > cwin->size_hints.max_width) *w = cwin->size_hints.max_width;
    if (*h > cwin->size_hints.max_height) *h = cwin->size_hints.max_height;
  }
}
...
static void get_protocols(WClientWin *cwin) {
  ...
  if (!XGetWMProtocols(wglobal.dpy, cwin->client_win, &protocols, &n)) return;
...
static void get_initial_winprops(Window win, int *frame_id_ret, int *ws_ret, int *flags) {
  int ws = -1, frame_id = 0;
  WWinProp *winprop;
  ...
  if (ws < WORKSPACE_STICKY || ws >= SCREEN->n_workspaces) ws = WORKSPACE_CURRENT;
...
static void get_colormaps(WClientWin *cwin) {
  ...
  n = do_get_property(wglobal.dpy, cwin->client_win, wglobal.atom_wm_colormaps, XA_WINDOW, 100L,
                      (uchar **)&wins);
...
static void clear_colormaps(WClientWin *cwin) {
  ...
  for (i = 0; i < cwin->n_cmapwins; i++) XSelectInput(wglobal.dpy, cwin->cmapwins[i], 0);
...
void install_cmap(Colormap cmap) {
  if (cmap == None) cmap = SCREEN->default_cmap;
...
again:
  ...
  for (i = cwin->n_cmapwins - 1; i >= 0; i--) {
    ...
    if (cwin->cmapwins[i] == cwin->client_win) found = TRUE;
  }
...
static void restore_gravity(WFrame *frame, WClientWin *cwin, int *xret, int *yret) {
  int grav = cwin->size_hints.win_gravity;
...
WClientWin *create_clientwin(Window win, int flags, int initial_state,
  ...
  cwin->icon_name = get_string_property(cwin->client_win, XA_WM_ICON_NAME, NULL);
  cwin->label = NULL;
  cwin->label_width = 0;
  ...
  if (cwin->orig_bw != 0) configure_cwin_bw(win, 0);
...
static WFrame *find_win_frame(Window win) {
  ...
  if (cwin != NULL && CWIN_HAS_FRAME(cwin)) return CWIN_FRAME(cwin);

  return NULL;
}

static bool add_clientwin(WClientWin *cwin, XWindowAttributes *attr, int mflags) {
  ...
  if (XGetTransientForHint(wglobal.dpy, win, &(cwin->transient_for))) {
    ...
    if (above == NULL) cwin->transient_for = None;
  }
  ...
  if (frame_id != 0) frame = find_frame_by_id(frame_id);

  if (frame == NULL) {
    if ((attr->x != 0 || attr->y != 0 || (cwin->size_hints.flags & (USPosition | PPosition)) != 0)
    ...
    if (!(mflags & MANAGE_RESPECT_POS)) {
      ...
      clientwin_to_frame_size(attr->width, attr->height, frameflags, &w, &h);
      calc_placement(w, h, ws, &(attr->x), &(attr->y));
    } else {
      account_gravity(frameflags, cwin, &(attr->x), &(attr->y));
    }

    frame = create_frame(attr->x, attr->y, attr->width, attr->height, frame_id, frameflags);
  ...
  if (!frame_attach_clientwin(frame, cwin)) return FALSE;
  ...
  if (!(mflags & MANAGE_INITIAL) &&
    ...
    if (frame->current_cwin != cwin) frame_switch_clientwin(frame, cwin);
...
again:
  ...
  if (hints != NULL && hints->flags & StateHint) state = hints->initial_state;
  ...
  if (!dock && state == WithdrawnState) {
    if (hints->flags & IconWindowHint && hints->icon_window != None) {
       ...
       */
      if (mflags & MANAGE_INITIAL) XUnmapWindow(wglobal.dpy, win);
      ...
      if (cwin != NULL) {
        if (WTHING_IS(cwin, WTHING_DOCKWIN)) return cwin;
  ...
  if (!dock &&
      (attr.override_redirect || (mflags & MANAGE_INITIAL && attr.map_state != IsViewable)))
    goto fail2;

  if (state != NormalState && state != IconicState) state = NormalState;

  /* Allocate and initialize */
  cwin = create_clientwin(win, 0, state, &attr);

  if (cwin == NULL) goto fail2;

  if (dock) {
    winprop = find_winprop_win(origwin);
    if (winprop != NULL) cwin->dockpos = winprop->dockpos;

    if (!add_dockwin(cwin)) goto failure;
  } else {
    if (!add_clientwin(cwin, &attr, mflags)) goto failure;
  }

  /* Check that the window exists. The previous check does not seem
   * to catch all cases of window destroyal.
   */
  if (XGetWindowAttributes(wglobal.dpy, win, &attr)) return cwin;
...
static void do_unmanage_clientwin(WClientWin *cwin, int action) {
  ...
  if (action != DESTROY) XRemoveFromSaveSet(wglobal.dpy, cwin->client_win);

  if (cwin->name != NULL) XFree((void *)cwin->name);

  if (cwin->icon_name != NULL) XFree((void *)cwin->icon_name);

  clientwin_unuse_label(cwin);
  if (cwin->label != NULL) free(cwin->label);
  ...
  if (cwin->orig_bw != 0) configure_cwin_bw(cwin->client_win, cwin->orig_bw);
 ...
 * be mapped (deinit)
 */
void unmanage_clientwin(WClientWin *cwin) { do_unmanage_clientwin(cwin, UNMANAGE); }

/* Used when the window was unmapped */
void unmap_clientwin(WClientWin *cwin) { do_unmanage_clientwin(cwin, UNMAP); }

/* Used when the window was destroyed */
void destroy_clientwin(WClientWin *cwin) { do_unmanage_clientwin(cwin, DESTROY); }

/* */

void iconify_clientwin(WClientWin *cwin) { D(warn("Ignoring request to iconify client window")); }

void clientwin_detach(WClientWin *cwin) { attachdetach_clientwin(NULL, cwin, FALSE, 0, 0); }

/* */

void kill_clientwin(WClientWin *cwin) { XKillClient(wglobal.dpy, cwin->client_win); }
...
void close_clientwin(WClientWin *cwin) {
  if (cwin->flags & CWIN_P_WM_DELETE) send_clientmsg(cwin->client_win, wglobal.atom_wm_delete);
#if 0
	else
		kill_clientwin(cwin);
#endif
...
void set_clientwin_size(WClientWin *cwin, int w, int h) {
  if (!CWIN_HAS_FRAME(cwin)) return; /* Resizing of docked windows is not supported. */
...
void attachdetach_clientwin(WFrame *frame, WClientWin *cwin, bool coords, int x, int y) {
  WFrame *oldframe = NULL;
  bool is_new = FALSE;
  int w = 0, h = 0;

  if (CWIN_HAS_FRAME(cwin)) oldframe = CWIN_FRAME(cwin);
  ...
  if (frame == NULL) {
    ...
    if (oldframe != NULL && oldframe->cwin_count == 1) {
      if (coords) set_frame_pos(oldframe, x, y);
      frame = oldframe;
      /* bring frame here */
...
static const char *clientwin_beg_label(WClientWin *cwin) {
  if (cwin->name != NULL) return cwin->name;
  if (cwin->icon_name != NULL) return cwin->icon_name;
...
#endif
  if (frame != NULL && WTHING_IS(frame, WTHING_FRAME)) frame_recalc_bar(frame);
}
