...
static bool do_create_dock(WDock *dock, int x, int y, int flags) {
  ...
  win = create_simple_window(x, y, dock->w, dock->h, GRDATA->base_colors.pixels[WCG_PIX_BG]);
...
void set_dock_params(const char *geom, const uint flags) {
  ...
  if (flag & XNegative) create_flags |= DOCK_RIGHT;
  if (flag & YNegative) create_flags |= DOCK_BOTTOM;
}
...
static WDock *create_dock(int x, int y, bool horiz) {
  ...
  if (create_flags & DOCK_RIGHT) create_x = SCREEN->width + create_x - DOCKWIN_W;
  if (create_flags & DOCK_BOTTOM) create_y = SCREEN->height + create_y - DOCKWIN_H;
...
static void calc_dockwin_xy(WDock *dock, WClientWin *cwin, int nth, int *xret, int *yret) {
  int h = dock->dock_h;

  *xret = (nth / h) * DOCKWIN_W;
  *yret = (nth % h) * DOCKWIN_H;

  if (cwin->client_w < DOCKWIN_W) *xret += (DOCKWIN_W - cwin->client_w) / 2;
  if (cwin->client_h < DOCKWIN_H) *yret += (DOCKWIN_H - cwin->client_h) / 2;
}
...
static void dock_resized(WDock *dock) {
  ...
  XMoveResizeWindow(wglobal.dpy, dock->win, dock->x, dock->y, dock->w, dock->h);
...
static void grow_dock(WDock *dock) {
  if (dock->flags & DOCK_HORIZONTAL) {
    dock->dock_w++;
    if (dock->flags & DOCK_RIGHT) dock->x -= DOCKWIN_W;
  } else {
    dock->dock_h++;
    if (dock->flags & DOCK_BOTTOM) dock->y -= DOCKWIN_H;
  }
...
static bool do_add_dockwin(WDock *dock, WClientWin *dw) {
  ...
  while (dw2 != NULL) {
    if (dw2->dockpos > dw->dockpos) break;
...
bool add_dockwin(WClientWin *dw) {
  ...
  if (dock == NULL) dock = create_dock(0, 0, FALSE);

  if (dock == NULL || !do_add_dockwin(dock, dw)) return FALSE;

  if (dock->dockwin_count == 1) map_winobj((WWinObj *)dock);
...
static void do_remove_dockwin(WDock *dock, WClientWin *dw, bool subdest) {
  ...
  if (dock->dockwin_count == 0 || subdest) return;
...
void remove_dockwin(WClientWin *dw) {
  ...
  if (dock->dockwin_count == 0 && !(dock->flags & WTHING_SUBDEST)) unmap_winobj((WWinObj *)dock);
}
...
void dock_toggle_hide() {
  ...
  if (!WWINOBJ_IS_HIDDEN(dock)) {
    if (dock->dockwin_count != 0) map_winobj((WWinObj *)dock);
  } else {
...
void dock_toggle_dir() {
  ...
  if (dock->flags & DOCK_RIGHT) dock->x = SCREEN->width - dock->h;

  if (dock->flags & DOCK_BOTTOM) dock->y = SCREEN->height - dock->w;
...
void set_dock_pos(WDock *dock, int x, int y) {
  ...
  if (dock->x + dock->w == SCREEN->width) dock->flags |= DOCK_RIGHT;
  if (dock->y + dock->h == SCREEN->height) dock->flags |= DOCK_BOTTOM;
