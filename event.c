...
void mainloop() {
  ...
  for (;;) {
    ...
    if (wglobal.input_mode == INPUT_MOVERES) keyboard_handler();
...
static void handle_expose(const XExposeEvent *ev) {
  ...
  while (XCheckWindowEvent(wglobal.dpy, ev->window, ExposureMask, &tmp)) /* nothing */
    ;
...
static void handle_map_request(const XMapRequestEvent *ev) {
  ...
  if (thing == NULL) {
    if (ev->parent == SCREEN->root) manage_clientwin(ev->window, 0);
    return;
  }
...
#ifndef CF_NO_WILD_WINDOWS
  if ((ev->value_mask & (CWX | CWY)) != 0) move_wildwin(cwin, ev->x, ev->y);
#endif
}
...
static void autoraise_handler() {
  if (wglobal.current_winobj == NULL) return;
...
static void handle_enter_window(XEvent *ev) {
  ...
  if (wglobal.input_mode == INPUT_CTXMENU) return;
...
#else
  while (XCheckMaskEvent(wglobal.dpy, EnterWindowMask, ev)) /* nothing */
    ;
#endif
  ...
  if (eev->window == eev->root) {
    /*if(eev->same_screen)
            return;*/
    if (eev->mode == NotifyInferior || wglobal.current_winobj == NULL) return;
    ...
    if (WTHING_IS(thing, WTHING_CLIENTWIN)) {
      if (CWIN_FRAME((WClientWin *)thing) == NULL) return;
    }
  }
...
static void handle_unmap_notify(const XUnmapEvent *ev) {
  ...
  if (WTHING_IS(thing, WTHING_CLIENTWIN)) unmap_clientwin((WClientWin *)thing);
}
...
static void handle_client_message(const XClientMessageEvent *ev) {
  ...
  if (ev->message_type != wglobal.atom_wm_change_state) return;

  cwin = find_clientwin(ev->window);

  if (cwin != NULL && ev->format == 32 && ev->data.l[0] == IconicState) {
    if (cwin->state == NormalState) iconify_clientwin(cwin);
  }
}
...
static bool current_cwin(WClientWin *cwin) {
  if (cwin->t_parent != (WThing *)wglobal.current_winobj) return FALSE;
...
static void handle_cwin_cmap(WClientWin *cwin, const XColormapEvent *ev) {
  ...
  if (ev->window == cwin->client_win) {
    ...
    if (current_cwin(cwin)) install_cwin_cmap(cwin);
  } else {
...
static void handle_focus_in(const XFocusChangeEvent *ev) {
  ...
  if (wglobal.current_winobj == winobj) return;
  ...
  if (winobj != NULL) activate(winobj);
}
...
static void handle_property(const XPropertyEvent *ev) {
  ...
  switch (ev->atom) {
    ...
    case XA_WM_ICON_NAME:
      ...
      if (cwin->name == NULL) need_recalc = TRUE;
      break;
...
void grab_kb_ptr() {
  ...
  XGrabPointer(wglobal.dpy, SCREEN->root, True, GRAB_POINTER_MASK, GrabModeAsync, GrabModeAsync,
               None, x_cursor(CURSOR_DEFAULT), CurrentTime);
  XGrabKeyboard(wglobal.dpy, SCREEN->root, False, GrabModeAsync, GrabModeAsync, CurrentTime);
...
void pointer_handler(XEvent *ev) {
  ...
  while (1) {
    XFlush(wglobal.dpy);
    get_event_mask(ev, GRAB_POINTER_MASK | ExposureMask | KeyPressMask | KeyReleaseMask |
                           EnterWindowMask | FocusChangeMask);

    switch (ev->type) {
      CASE_EVENT(ButtonRelease)
      if (handle_button_release(&(ev->xbutton))) break;
      continue;
...
void keyboard_handler() {
  ...
  while (1) {
    ...
    if (wglobal.input_mode != INPUT_MOVERES) break;
  }
