...
static bool wait_exit() {
  ...
  if (wglobal.n_alive == 0) return TRUE;
  ...
  while (1) {
    sleep(1);
    if (wglobal.n_alive == 0) return TRUE;
    if (timeout > time(NULL)) return FALSE;
  }
}
...
static void terminate_chld() {
  ...
  for (i = 0; i < wglobal.n_children; i++) {
    if (wglobal.children[i] != 0) kill(wglobal.children[i], SIGTERM);
  }

  if (wait_exit()) return;

  if (wglobal.children[i] != 0) kill(wglobal.children[i], SIGKILL);
...
void wm_exit() { wm_exitret(EXIT_SUCCESS); }
