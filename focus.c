...
void do_set_focus(WThing *thing) {
  ...
  if (WTHING_IS_UNFOCUSABLE(thing)) return;
...
void set_focus_weak(WThing *thing) {
  if (wglobal.focus_next == NULL) do_set_focus(thing);
}

void set_focus(WThing *thing) { wglobal.focus_next = thing; }

static int sign(int i) { return (i > 0 ? 1 : i < 0 ? -1 : 0); }

static bool try_focus(WThing *next) {
  if (WTHING_IS(next, WTHING_WINOBJ) && !WTHING_IS_UNFOCUSABLE(next) &&
...
static WWinObj *do_circulate(int dir) {
  ...
  if (tmp == NULL) {
    ...
    if (tmp == NULL || try_focus(tmp)) return (WWinObj *)tmp;
  }
  ...
  do {
    ...
    if (next == NULL || next == tmp) break;

    if (try_focus(next)) return (WWinObj *)next;
  } while (1);

  return NULL;
}

WWinObj *circulate(int dir) { return do_circulate(dir); }

void circulateraise(int dir) {
  WWinObj *obj = do_circulate(dir);

  if (obj != NULL) raise_winobj(obj);
}
...
void goto_previous() {
  ...
  if (obj != NULL) {
    if (!on_current_workspace(obj)) dodo_switch_workspace(workspace_of(obj));
