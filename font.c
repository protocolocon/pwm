...
XFontStruct *load_font(Display *dpy, const char *fontname) {
  ...
  if (xfnt == NULL) {
    warn("Could not load font \"%s\", trying \"%s\"", fontname, CF_FALLBACK_FONT_NAME);
...
char *make_label(XFontStruct *fnt, const char *str, const char *trailer, int maxw, int *wret) {
  ...
  if (len > 0) {
    while (--len) {
      w = XTextWidth(fnt, str, len);
      if (tw + w + dw <= maxw) break;
    }
  }

  if (wret != NULL) *wret = tw + w + dw;
