...
static bool do_frame_detach_clientwin(WFrame *frame, WClientWin *cwin, WFrame *transient_dest);
...
static void hideshow_transients(WFrame *dest, Window show_for, Window hide_for);
...
void clientwin_to_frame_size(int iw, int ih, int flags, int *fw, int *fh) {
  ...
  if (!(flags & WFRAME_NO_BAR)) ih += GRDATA->bar_height;
...
WFrame *create_frame(int x, int y, int iw, int ih, int id, int flags) {
  ...
  frame->frame_iw = iw;
  frame->frame_ih = ih;
  frame->frame_ix = frame->frame_iy = (flags & WFRAME_NO_BORDER ? 0 : CF_BORDER_WIDTH);
  ...
  frame_win =
      XCreateWindow(wglobal.dpy, SCREEN->root, FRAME_X(frame), FRAME_Y(frame), FRAME_W(frame),
                    FRAME_H(frame), 0, CopyFromParent, InputOutput, CopyFromParent, 0, NULL);
#else
  frame_win = create_simple_window(FRAME_X(frame), FRAME_Y(frame), FRAME_W(frame), FRAME_H(frame),
                                   GRDATA->base_colors.pixels[WCG_PIX_BG]);
#endif
  bar_win = create_simple_window(BAR_X(frame), BAR_Y(frame), BAR_W(frame), GRDATA->bar_height,
...
WFrame *create_add_frame_simple(int root_x, int root_y, int iiw, int iih) {
  ...
  if (frame != NULL) add_winobj((WWinObj *)frame, WORKSPACE_CURRENT, LVL_NORMAL);
...
static void do_frame_switch_clientwin(WFrame *frame, WClientWin *cwin, bool simple) {
  if (!WFRAME_IS_NORMAL(frame)) {
    frame->current_cwin = cwin;
    if (!simple && WFRAME_IS_SHADE(frame)) draw_frame_bar(frame, TRUE);
  ...
  if (!simple) hide_clientwin(frame->current_cwin);

  hideshow_transients(frame, cwin->client_win, simple ? None : frame->current_cwin->client_win);

  frame->current_cwin = cwin;

  if (IS_CURRENT_FRAME(frame)) set_focus_weak((WThing *)cwin);

  if (!simple) draw_frame_bar(frame, TRUE);

  update_winlist();
}

void frame_switch_clientwin(WFrame *frame, WClientWin *cwin) {
  if (cwin != frame->current_cwin) do_frame_switch_clientwin(frame, cwin, FALSE);
}
...
void frame_switch_nth(WFrame *frame, int num) {
  ...
  if (thing != NULL) frame_switch_clientwin(frame, (WClientWin *)thing);
}
...
void set_frame_state(WFrame *frame, int stateflags) {
  ...
  */
  ...
  if (stateflags & WFRAME_HIDDEN) {
    /* hide it */
    if (changes & WFRAME_HIDDEN) unmap_winobj((WWinObj *)frame);
    ...
    if (IS_CURRENT_FRAME(frame)) set_focus_weak((WThing *)frame);
  } else {
    /* show it */
    if (WWINOBJ_IS_MAPPED(frame))
...
static void restack_transient(WFrame *frame, Window transient_for) {
  ...
  if (cwin == NULL || !CWIN_HAS_FRAME(cwin)) return;

  restack_winobj_above((WWinObj *)frame, (WWinObj *)CWIN_FRAME(cwin), FALSE);
}
...
static void do_frame_attach_clientwin(WFrame *frame, WClientWin *cwin) {
  ...
  set_integer_property(cwin->client_win, wglobal.atom_frame_id, frame->frame_id);
  set_integer_property(cwin->client_win, wglobal.atom_workspace_num, frame->workspace);

  check_minmax_size(frame, cwin);
  if (frame->frame_iw != cwin->client_w || frame->frame_ih != cwin->client_h)
    frame_clientwin_resize(frame, cwin, frame->frame_iw, frame->frame_ih, FALSE);
  else
  ...
  if (cwin->state != IconicState && WFRAME_IS_SHADE(frame)) {
    ...
    if (cwin->state == IconicState) set_frame_state(frame, frame->flags | WFRAME_SHADE);
    frame_switch_clientwin(frame, cwin);
  }

  if (!WFRAME_IS_NORMAL(frame) || cwin != frame->current_cwin) hide_clientwin(cwin);
}
...
bool frame_attach_clientwin(WFrame *frame, WClientWin *cwin) {
  ...
  if (CWIN_HAS_FRAME(cwin)) {
    if (CWIN_FRAME(cwin) == frame) return TRUE;
  ...
  XSelectInput(wglobal.dpy, cwin->client_win, cwin->event_mask & ~StructureNotifyMask);
  XReparentWindow(wglobal.dpy, cwin->client_win, frame->frame_win, frame->frame_ix,
                  frame->frame_iy);
  ...
  if (oframe != NULL) {
    if (!do_frame_detach_clientwin(oframe, cwin, frame)) oframe = NULL;
  }
  ...
  if (oframe != NULL && oframe->cwin_count == 1) {
    ...
    if (cwin->transient_for != None) restack_transient(oframe, cwin->transient_for);

#ifndef CF_NO_WILD_WINDOWS
    if (CWIN_IS_WILD(cwin)) frame_set_decor(oframe, WFRAME_NO_DECOR);
#endif
  }
...
static bool do_frame_detach_clientwin(WFrame *frame, WClientWin *cwin, WFrame *transient_dest) {
...
#else
  next = next_clientwin(cwin);
  if (next == NULL) next = prev_clientwin(cwin);
#endif
  ...
  if (frame->cwin_count == 0) {
    if (!(frame->flags & WTHING_SUBDEST)) destroy_frame(frame);
    return FALSE;
  }
  ...
  if (!FRAME_MAXW_UNSET(frame) && frame->frame_iw > frame->max_w) nw = frame->max_w;
  if (!FRAME_MAXH_UNSET(frame) && frame->frame_ih > frame->max_h) nh = frame->max_h;

  if (cwin == frame->current_cwin) do_frame_switch_clientwin(frame, next, TRUE);
...
void frame_detach_clientwin(WFrame *frame, WClientWin *cwin, int x, int y) {
  ...
  if (do_frame_detach_clientwin(frame, cwin, NULL) && frame->cwin_count == 1) {
    ...
    if (cwin->transient_for != None) restack_transient(frame, cwin->transient_for);

#ifndef CF_NO_WILD_WINDOWS
    if (CWIN_IS_WILD(cwin)) frame_set_decor(frame, WFRAME_NO_DECOR);
#endif
  }
...
static void do_set_frame_size(WFrame *frame, int iw, int ih) {
  ...
  XResizeWindow(wglobal.dpy, frame->frame_win, FRAME_W(frame), FRAME_H(frame));
...
static void check_minmax_size(WFrame *frame, WClientWin *cwin) {
  if (cwin->size_hints.flags & PMinSize) {
    if (cwin->size_hints.min_width > frame->min_w) frame->min_w = cwin->size_hints.min_width;
    if (cwin->size_hints.min_height > frame->min_h) frame->min_h = cwin->size_hints.min_height;
  }
  ...
  if (!FRAME_MAXW_UNSET(frame) && frame->min_w > frame->max_w) frame->max_w = frame->min_w;

  if (!FRAME_MAXH_UNSET(frame) && frame->min_h > frame->max_h) frame->max_h = frame->min_h;
}
 ...
 */
void frame_clientwin_resize(WFrame *frame, WClientWin *cwin, int w, int h, bool dmax) {
  int fw, fh;
  WClientWin *tmp;
  ...
  if (frame->cwin_count > 1) {
    ...
    for (tmp = first_clientwin((WThing *)frame); tmp != NULL; tmp = next_clientwin(tmp)) {
      if (tmp == cwin) continue;
      if (tmp->client_w > fw) fw = tmp->client_w;
      if (tmp->client_h > fh) fh = tmp->client_h;
    }
  }

  do_set_frame_size(frame, fw, fh);

  if (!dmax) frame->flags = tmpf;
...
void frame_toggle_maximize(WFrame *frame, int mask) {
  ...
  if (mask == 0) mask = WFRAME_MAX_BOTH;
...
static void hideshow_transients(WFrame *frame, Window show_for, Window hide_for) {
  WWinObj *obj;
  WClientWin *cwin;

  if (hide_for == show_for) return;

  obj = frame->stack_above_list;

  while (obj != NULL) {
    if (obj->type != WTHING_FRAME) goto cont;
...
static void move_transients_to(WFrame *dest, WFrame *src, Window transient_for) {
  ...
  for (obj = src->stack_above_list; obj != NULL; obj = next) {
    ...
    if (obj->type != WTHING_FRAME) continue;
    ...
    if (cwin->transient_for != transient_for) continue;
    ...
    if (dest == NULL) {
      ...
      if (dest->cwin_count > 0) set_frame_state((WFrame *)obj, obj->flags | WFRAME_HIDDEN);
    }
  }
}
...
static void do_attach_tagged(WFrame *frame) {
  ...
  for (; cwin != NULL; cwin = cwin->s_cwin_next) {
    if (!(cwin->flags & CWIN_TAGGED)) continue;

    cwin->flags &= ~CWIN_TAGGED;

    if (frame == NULL) {
      frame = create_add_frame_simple(0, 0, cwin->client_w, cwin->client_h);
...
void frame_attach_tagged(WFrame *frame) { do_attach_tagged(frame); }

void join_tagged() { do_attach_tagged(NULL); }
...
void frame_recalc_bar(WFrame *frame) {
  ...
  if (frame->flags & WFRAME_NO_BAR) return;

  for (cwin = first_clientwin((WThing *)frame); cwin != NULL; cwin = next_clientwin(cwin)) {
    if (cwin->state != NormalState && cwin->state != IconicState) continue;
  ...
  if (maxw < CF_BAR_MIN_WIDTH && frame->w >= CF_BAR_MIN_WIDTH) maxw = CF_BAR_MIN_WIDTH;
  ...
  if (tmp > 0) {
    ...
    if (tmp > CF_TAB_MAX_TEXT_X_OFF) tmp = CF_TAB_MAX_TEXT_X_OFF;

    w = tmaxw + tmp * 2;

    if (w < CF_TAB_MIN_WIDTH) {
      w = CF_TAB_MIN_WIDTH;
      if (w * n > maxw) w = maxw / n;
    }
  } else {
...
void frame_toggle_shade(WFrame *frame) { set_frame_state(frame, frame->flags ^ WFRAME_SHADE); }

void frame_toggle_sticky(WFrame *frame) {
  int ws = WORKSPACE_STICKY;

  if (WWINOBJ_IS_STICKY(frame)) ws = WORKSPACE_CURRENT;
...
void frame_set_decor(WFrame *frame, int decorflags) {
  ...
  if (flagchg & WFRAME_NO_BAR) {
    if (frame->flags & WFRAME_NO_BAR) {
      ...
      if (WFRAME_IS_SHADE(frame)) set_frame_state(frame, frame->flags & ~WFRAME_SHADE);
  ...
  XMoveResizeWindow(wglobal.dpy, frame->frame_win, frame->x, frame->y + frame->bar_h, w, h);

  if (flagchg & WFRAME_NO_BAR && !(decorflags & WFRAME_NO_BAR)) frame_recalc_bar(frame);
...
void frame_toggle_decor(WFrame *frame) {
  ...
  if ((frame->flags & WFRAME_NO_DECOR) == 0) decorflags = WFRAME_NO_DECOR;
