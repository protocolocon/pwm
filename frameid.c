...
static void reorder_frame_ids() {
  ...
  for (frame = (WFrame *)subthing((WThing *)SCREEN, WTHING_FRAME); frame != NULL;
    ...
    if (frame->frame_id < FRAME_ID_START_CLIENT) continue;
...
int new_frame_id() {
  if (((frame_id_cntr - FRAME_ID_START_CLIENT) & FRAME_ID_REORDER_INTERVAL) ==
          FRAME_ID_REORDER_INTERVAL - 1 ||
      frame_id_cntr == INT_MAX)
    reorder_frame_ids();
...
int use_frame_id(int id) {
  if (id > frame_id_cntr) frame_id_cntr = id;
...
void free_frame_id(int id) {
  if (frame_id_cntr == INT_MAX) reorder_frame_ids();
...
WFrame *find_frame_by_id(int id) {
  ...
  for (frame = (WFrame *)subthing((WThing *)SCREEN, WTHING_FRAME); frame != NULL;
       frame = (WFrame *)next_thing((WThing *)frame, WTHING_FRAME)) {
    if (frame->frame_id == id) return frame;
  }
