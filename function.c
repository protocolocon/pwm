...
static WFuncClass fclass_frame = {frame_handler, NULL, NULL, ARGTYPE_NONE};

static WFuncClass fclass_frame_i = {frame_i_handler, NULL, NULL, ARGTYPE_INT};

static WFuncClass fclass_clientframe = {frame_handler, NULL, NULL, ARGTYPE_NONE};

static WFuncClass fclass_clientframe_i = {frame_i_handler, NULL, NULL, ARGTYPE_INT};

static WFuncClass fclass_winobj = {winobj_handler, NULL, NULL, ARGTYPE_NONE};

static WFuncClass fclass_winobj_i = {winobj_i_handler, NULL, NULL, ARGTYPE_INT};

static WFuncClass fclass_global = {global_handler, NULL, NULL, ARGTYPE_NONE};

static WFuncClass fclass_global_i = {global_i_handler, NULL, NULL, ARGTYPE_INT};

static WFuncClass fclass_global_s = {global_s_handler, NULL, NULL, ARGTYPE_STRING};

static WFuncClass fclass_cwin = {cwin_handler, NULL, NULL, ARGTYPE_NONE};

static WFuncClass fclass_drag = {NULL, drag_handler, drag_end, ARGTYPE_NONE};

static WFuncClass fclass_show_menu = {show_menu_handler, NULL, show_menu_but_handler,
                                      ARGTYPE_STRING};

static WFuncClass fclass_menu_cmd = {menu_i_handler, NULL, NULL, ARGTYPE_NONE};
...
WFunction *lookup_func(const char *name, int arg_type) {
  ...
  while (func->fclass != NULL) {
    ...
    if (func->fclass->arg_type != arg_type) return NULL;
...
static void winobj_gen_handler(WThing *thing, WFunction *func, WFuncArg arg, int type) {
  ...
  if ((winobj = winobj_of(thing)) == NULL) return;

  if (!WTHING_IS(winobj, type)) return;
...
static void winobj_gen_i_handler(WThing *thing, WFunction *func, WFuncArg arg, int type) {
...
void show_menu_but_handler(WThing *thing, XButtonEvent *ev, WFunction *func, WFuncArg arg) {
  WMenuData *mdata;
...
static void wrap_switch_clientwin(WClientWin *cwin) {
  ...
  if (!WFRAME_IS_SHADE(CWIN_FRAME(cwin))) set_frame_state(CWIN_FRAME(cwin), 0);
}
