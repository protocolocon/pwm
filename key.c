...
void handle_keypress(XKeyEvent *ev) {
  ...
  if (wglobal.input_mode == INPUT_MOVERES) {
    ...
    if (thing == NULL) {
      ...
      if (thing == NULL) thing = find_thing(ev->window);

      if (thing == NULL) thing = (WThing *)SCREEN;
    }

    if (WTHING_IS(thing, WTHING_MENU)) actx = ACTX_MENU;
  ...
  if (binding != NULL && binding->func != NULL) {
    func = binding->func;
    if (func->fclass->handler != NULL) func->fclass->handler(thing, func, binding->arg);
  }
}
