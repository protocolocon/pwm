...
static void print_help(const OptParserOpt *opts, bool midlong, const OptParserCommonInfo *cinfo);

/* */

void optparser_init(int argc, char *const argv[], int mode, const OptParserOpt *opts,
...
int optparser_get_opt() {
  ...
  if (oid <= 0 || (oid & OPT_ID_RESERVED_FLAG) == 0) return oid;

  switch (oid) {
    case OPT_ID_ABOUT:
      if (o_cinfo->about != NULL) printf("%s", o_cinfo->about);
      break;

    case OPT_ID_VERSION:
      if (o_cinfo->version != NULL) printf("%s\n", o_cinfo->version);
      break;
...
static void print_opt(const OptParserOpt *opt, bool midlong, uint maxw, uint tw) {
  FILE *f = stdout;
  const char *p, *p2, *p3;
  uint w = 0;
...
static void print_help(const OptParserOpt *opts, bool midlong, const OptParserCommonInfo *cinfo) {
  ...
  while (1) {
    ...
    if (tmp != p) writef(stdout, p, tmp - p);
