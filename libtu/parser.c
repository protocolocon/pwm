...
enum { P_NONE = 1, P_EOF, P_STMT, P_STMT_NS, P_STMT_SECT, P_BEG_SECT, P_END_SECT };

/* */

static bool opt_include(Tokenizer *tokz, int n, Token *toks);

static ConfOpt common_opts[] = {{"include", "s", opt_include, NULL}, {NULL, NULL, NULL, NULL}};
  ...
  while (1) {
    ...
    switch (t) {
      ...
      case P_STMT_SECT:
        ...
        options = lookup_option(tokz->optstack[tokz->nest_lvl], TOK_IDENT_VAL(tokens + 0));
        if (options == NULL) options = lookup_option(common_opts, TOK_IDENT_VAL(tokens + 0));
...
static int check_argument(const char **pret, Token *tok, const char *p, bool si) {
  int mode;
...
bool do_check_args(const Tokenizer *tokz, Token *tokens, int ntokens, const char *fmt, bool si) {
  int i;
  int e;

  if (fmt == NULL) {
    if (ntokens != 1) tokz_warn_error(tokz, tokens[0].line, E_TOKZ_TOO_MANY_ARGS);
    return ntokens == 1;
  }
...
bool check_args(const Tokenizer *tokz, Token *tokens, int ntokens, const char *fmt) {
  return do_check_args(tokz, tokens, ntokens, fmt, FALSE);
}

bool check_args_loose(const Tokenizer *tokz, Token *tokens, int ntokens, const char *fmt) {
...
static bool try_include_dir(Tokenizer *tokz, const char *dir, int dlen, const char *file) {
  char *tmpname;
  bool retval;
...
extern void tokz_set_includepaths(Tokenizer *tokz, char **paths) { tokz->includepaths = paths; }
