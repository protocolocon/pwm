    ...
    if (*p != '%') {
      ...
      switch (fmt_spec) {
          ...
          switch (fmt_spec) {
            ...
            case 's':
              ...
              /* truncate string if necessary as requested by precision */
                ...
                const char *q =
                    memchr(str_arg, '\0', precision <= 0x7fffffff ? precision : 0x7fffffff);
        ...
        case 'p': {
...
#endif
          if (fmt_spec == 'd') {
            if (force_sign && arg_sign >= 0) tmp[str_arg_l++] = space_for_positive ? ' ' : '+';
          ...
          }
          {
            ...
            if (num_of_digits < precision) number_of_zeros_to_pad = precision - num_of_digits;
          }
      ...
      if (number_of_zeros_to_pad <= 0) {
        ...
        if (n > 0) {
          if (str_l < str_m) {
            ssize_t avail = str_m - str_l;
            fast_memcpy(str + str_l, str_arg, (int)(n > avail ? avail : n));
          }
          str_l += n;
        }
