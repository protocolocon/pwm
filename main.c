...
static bool initialize(const char *display, const char *cfgfile, bool onescreen);
 ...
 */
static OptParserOpt opts[] = {{OPT_ID('d'), "display", OPT_ARG, "host:dpy.scr", "X display to use"},
                              {'c', "cfgfile", OPT_ARG, "config_file", "Configuration file"},
                              {OPT_ID('o'), "onescreen", 0, NULL, "Manage default screen only"},
                              {0, NULL, 0, NULL, NULL}};

static const char wm_usage_tmpl[] = "Usage: $p [options]\n\n$o\n";

static const char wm_about[] = "PWM " PWM_VERSION
                               ", copyright (c) Tuomo Valkonen 1999-2001.\n"
                               "This program may be copied and modified under the terms of the "
                               "Artistic License or the GNU GPL.\n";

static OptParserCommonInfo wm_cinfo = {PWM_VERSION, wm_usage_tmpl, wm_about};
...
static bool initialize(const char *display, const char *cfgfile, bool onescreen) {
  ...
  if (dpy == NULL) die("Could not connect to X display '%s'", XDisplayName(display));
  ...
  if (!onescreen && nscr > 1) {
    ...
    if (wglobal.children == NULL) die_err();
  ...
  if (!preinit_screen(scr)) return FALSE;
...
static pid_t fork_screen(int scr) {
  ...
  if (pid == 0) {
    ...
    warn("Failed to fork process for screen %d: %s", scr, strerror(errno));
  } else {
