...
static int menu_width(WMenuData *mdata, int flags) {
  ...
  if (!(flags & WMENU_NOTITLE))
    maxw =
        textwidth(GRDATA->font, mdata->title) + 2 * CF_MENUTITLE_H_SPACE + GRDATA->stick_pixmap_w;

  for (; i < mdata->nentries; i++, ent++) {
    w = textwidth(GRDATA->menu_font, ent->name);
    w += 2 * CF_MENUENT_H_SPACE;
    if (ent->flags & WMENUENT_SUBMENU) inc = GRDATA->submenu_ind_w + CF_SUBMENU_IND_H_SPACE;
    if (w > maxw) maxw = w;
  }
...
static int entry_at(WMenu *menu, int x, int y) {
  ...
  if (x < 0 || x > menu->w) return NO_ENTRY;

  if (y < 0 || y > menu->h) return NO_ENTRY;

  y -= menu->title_height + CF_MENU_V_SPACE;

  if (y < 0) return ENTRY_TITLE;

  ent = y / menu->entry_height;

  if (ent >= menu->data->nentries) ent = NO_ENTRY;
...
static void submenu_pos(WMenu *menu, int entry, int *xret, int *yret) {
  ...
  x = menu->x + menu->w;
  y = menu->y + menu->title_height + CF_MENU_V_SPACE + entry * menu->entry_height;
 ...
 */
static WMenu *create_menu(WMenuData *mdata, WThing *context, int x, int y, WMenu *parent) {
  int w, h, th, eh;
  int flags = 0;
  Window win;
  WMenu *menu;

  if (mdata->init_func != NULL && mdata->nref == 0) mdata->init_func(mdata, context);

  if (mdata->flags & WMENUDATA_CONTEXTUAL) {
    if (context == NULL || WTHING_IS(context, WTHING_SCREEN) || WTHING_IS(context, WTHING_MENU))
      return NULL;
  ...
  if (mdata->nref != 0) flags |= (WMENU_NOTITLE | WMENU_CONTEXTUAL);

  if (mdata->title == NULL) flags |= (WMENU_NOTITLE | WMENU_CONTEXTUAL);

  if (parent != NULL) flags |= parent->flags & (WMENU_NOTITLE | WMENU_CONTEXTUAL);
  ...
  if (flags & WMENU_NOTITLE) th = 0;

  h += th;

  win = create_simple_window(x, y, w, h, GRDATA->base_colors.pixels[WCG_PIX_BG]);
  ...
  if (mdata->nref++ == 0) mdata->inst1 = menu;
...
void update_menus(WMenuData *mdata) {
  ...
  while (menu != NULL) {
    if (menu->data == mdata) update_menu(menu);
...
static void do_destroy_menu(WMenu *menu) {
  ...
  if (menu->data->inst1 == menu) menu->data->inst1 = NULL;
 ...
 */
static void do_destroy_menu_tree(WMenu *menu, bool setsel, bool contextual_only) {
  ...
  for (; p != NULL; p = next) {
    ...
    if (!WTHING_IS(p, WTHING_MENU)) continue;

    if (contextual_only && !(p->flags & WMENU_CONTEXTUAL)) continue;
...
void destroy_menu_tree(WMenu *menu) { do_destroy_menu_tree(menu, TRUE, FALSE); }

/* Destroy a complete menu tree starting from the root.
 */
static void destroy_menu_root(WMenu *menu, bool all) {
  do {
    if (menu->stack_above == NULL) break;

    if (!all && menu->stack_above->flags & WMENU_KEEP) break;
...
void destroy_contextual_menus() {
  ...
  while (obj != NULL) {
    ...
    if (WTHING_IS(obj, WTHING_MENU)) do_destroy_menu_tree((WMenu *)obj, TRUE, TRUE);
...
static void finish_contextual_menus() {
  if (wglobal.input_mode == INPUT_CTXMENU) ungrab_kb_ptr();
...
static void menu_set_selected(WMenu *menu, int selected) {
  ...
  if (menu->data->nentries <= selected) selected = NO_ENTRY;

  if (MENU_SUBMENU(menu) != NULL) do_destroy_menu_tree(MENU_SUBMENU(menu), FALSE, FALSE);

  if (menu->selected == selected) return;
...
static WMenu *show_selected_submenu(WMenu *menu) {
  ...
  if (menu->selected < 0 || MENU_SUBMENU(menu) != NULL) return MENU_SUBMENU(menu);

  entry = &(menu->data->entries[menu->selected]);

  if (!(entry->flags & WMENUENT_SUBMENU)) return NULL;
...
static bool menu_execute_selected(WMenu *menu) {
  ...
  if (menu->selected < 0) return TRUE;

  /* Submenu already visible? -> return */
  if (MENU_SUBMENU(menu) != NULL) return TRUE;
  ...
  if (data->exec_func != NULL) data->exec_func(entry, context);

  if (--data->nref == 0 && data->deinit_func != NULL) data->deinit_func(data);
...
static void keep(WMenu *menu) {
  ...
  if (menu->flags & (WMENU_KEEP | WMENU_CONTEXTUAL)) return;
...
static void move_menu(WMenu *menu, int dx, int dy) {
  ...
  while (p != NULL) {
    if (WTHING_IS(p, WTHING_MENU)) do_move((WMenu *)p, dx, dy);
    p = traverse_winobjs(p, (WWinObj *)menu);
  }
}

void set_menu_pos(WMenu *menu, int x, int y) { move_menu(menu, x - menu->x, y - menu->y); }

/* */

void activate_menu(WMenu *menu) { draw_menu(menu, TRUE); }

void deactivate_menu(WMenu *menu) { draw_menu(menu, TRUE); }
  ...
  switch (cmd) {
    ...
    case MENU_CMD_EXECUTE:
      ...
      if (other != NULL) {
        ...
        if (other->selected == NO_ENTRY) other->selected = 0;
        vis_entry(other, other->selected);
        break;
      }

      if (cmd != MENU_CMD_EXECUTE) break;
    ...
    case MENU_CMD_LEAVESUB:
      ...
      if (other != NULL) {
         ...
         */
        if (wglobal.input_mode == INPUT_CTXMENU) wglobal.grab_holder = (WThing *)other;
        vis_entry(other, other->selected);
      }
      break;
...
void show_menu(WMenuData *mdata, WThing *context, int x, int y, bool button_down) {
  WMenu *menu;

  if (mdata->inst1 != NULL) {
    menu = mdata->inst1;
    if (menu->flags & WMENU_KEEP) goto create_it;
...
void menu_button(WThing *thing, XButtonEvent *ev, WFunction *func, WFuncArg arg) {
  WMenu *menu;
  int x, y, entry;

  if (!WTHING_IS(thing, WTHING_MENU)) return;
  ...
  /* release */
  if (entry >= 0) {
    if (menu->selected == entry) menu_execute_selected(menu);
...
bool menu_select_at(WMenu *menu, int rx, int ry) {
  ...
  if (entry == NO_ENTRY) {
    if (MENU_SUBMENU(menu) == NULL) menu_set_selected(menu, NO_ENTRY);
...
void menu_motion(WThing *thing, XMotionEvent *ev, int dx, int dy, WFunction *func, WFuncArg arg) {
  WMenu *menu;
  int ret;
  ...
  if (!menu_select_at(menu, ev->x_root, ev->y_root)) {
    ...
    if (thing == (WThing *)menu || thing == NULL || !WTHING_IS(thing, WTHING_MENU)) {
      end_scroll();
      return;
    }
  ...
  if (test_scroll(menu, ev->x_root, ev->y_root)) start_scroll(menu);
}
...
static void scrollfunc() {
  ...
  if (menu_select_at(menu, dx, dy)) {
    if (test_scroll(menu, dx, dy)) return;
  } else if (test_scroll(menu, dx, dy)) {
    return;
  } else {
    if (!find_window_at(dx, dy, &win)) goto end;

    menu = (WMenu *)find_thing_t(win, WTHING_MENU);

    if (menu == NULL) goto end;

    if (test_scroll(menu, dx, dy)) start_scroll(menu);
  }
...
static bool test_scroll(WMenu *menu, int rx, int ry) {
  if (rx <= 0 && menu->x < 0)
    scrollhoriz = 1;
  else if (rx >= SCREEN->width - 1 && menu->x + menu->w >= SCREEN->width - SCROLL_BORDER)
    scrollhoriz = -1;
  else
    scrollhoriz = 0;
  ...
  if (ry <= 0 && menu->y < 0)
    scrollvert = 1;
  else if (ry >= SCREEN->height - 1 && menu->y + menu->h >= SCREEN->height - SCROLL_BORDER)
    scrollvert = -1;
  else
    scrollvert = 0;
...
static void vis_entry(WMenu *menu, int entry) {
  ...
  if (is_vis_entry(menu, entry, &dx, &dy)) return;
