...
bool register_menudata(WMenuData *menudata) {
  ...
  if (menudata->flags & WMENUDATA_USER) menudata->exec_func = user_menudata_exec_func;
...
void free_user_menudata(WMenuData *menudata) {
  ...
  if (menudata->title != NULL) free(menudata->title);
  ...
  for (i = 0; i < menudata->nentries; i++, entry++) {
    ...
    if (entry->u.f.arg_type == ARGTYPE_STRING) free(ARG_TO_STRING(entry->u.f.arg));
  }
...
static void user_menudata_exec_func(WMenuEnt *entry, WThing *context) {
  ...
  if (entry->u.f.function == NULL) return;

  fclass = entry->u.f.function->fclass;

  if (fclass == NULL || fclass->handler == NULL) return;
