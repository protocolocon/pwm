 ...
 */
static bool smart_placement(int x, int y, int pw, int ph, int w, int h, int *xret, int *yret) {
  int hs1, vs1, hs2, vs2;
  int rnum = rand();
  ...
  if (hs1 >= w) {
     ...
     */
    if (vs1 >= h && rnum & RMASK3) goto vplace;
...
static WWinObj *is_occupied(int x, int y, int w, int h, int ws) {
  ...
  for (current = (WWinObj *)subthing((WThing *)SCREEN, WTHING_WINOBJ); current != NULL;
       current = (WWinObj *)next_thing((WThing *)current, WTHING_WINOBJ)) {
    if (current->workspace != ws && current->workspace != WORKSPACE_STICKY) continue;

    if (!WWINOBJ_IS_MAPPED(current)) continue;
...
static int next_least_x(int x, int ws) {
  ...
  for (p = (WWinObj *)subthing((WThing *)SCREEN, WTHING_WINOBJ); p != NULL;
    ...
    if (px + pw > x && px + pw < retx) retx = px + pw;
  }
...
static int next_lowest_y(int y, int ws) {
  ...
  for (p = (WWinObj *)subthing((WThing *)SCREEN, WTHING_WINOBJ); p != NULL;
    ...
    if (py + ph > y && py + ph < rety) rety = py + ph;
  }
...
static bool flaccid_placement(int w, int h, int ws, int *xret, int *yret) {
  ...
  while (x < SCREEN->width) {
    ...
    while (p != NULL && y + h < SCREEN->height) {
      y = p->y + 1;
      y += (WTHING_IS(p, WTHING_FRAME) && WFRAME_IS_SHADE(p)) ? BAR_H((WFrame *)p) : p->h;
      p = is_occupied(x, y, w, h, ws);
    }
  ...
  while (y < SCREEN->height) {
    ...
    while (p != NULL && x + w < SCREEN->width) {
      x = p->x + 1;
      x += (WTHING_IS(p, WTHING_FRAME) && WFRAME_IS_SHADE(p)) ? BAR_W((WFrame *)p) : p->w;
      p = is_occupied(x, y, w, h, ws);
    }
...
void calc_placement(int w, int h, int ws, int *xret, int *yret) {
  ...
  if (ws < 0) ws = SCREEN->current_workspace;

  if (flaccid_placement(w, h, ws, xret, yret)) return;

  if (current != NULL) {
    /* Try to place it so that it doesn't overlap current winobj. */
    if (smart_placement(current->x, current->y, current->w, current->h, w, h, xret, yret)) return;
    ...
    if (smart_placement(x - CF_DRAG_TRESHOLD, y - CF_DRAG_TRESHOLD, CF_DRAG_TRESHOLD,
                        CF_DRAG_TRESHOLD, w, h, xret, yret))
      return;
