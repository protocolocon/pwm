...
static bool motion_in_treshold(int x, int y) {
  return (x > p_x - CF_DRAG_TRESHOLD && x < p_x + CF_DRAG_TRESHOLD && y > p_y - CF_DRAG_TRESHOLD &&
          y < p_y + CF_DRAG_TRESHOLD);
}

bool find_window_at(int x, int y, Window *childret) {
  int dstx, dsty;

  if (!XTranslateCoordinates(wglobal.dpy, SCREEN->root, SCREEN->root, x, y, &dstx, &dsty, childret))
    return FALSE;

  if (*childret == None) return FALSE;
...
static void call_button(WBinding *binding, XButtonEvent *ev, bool nohand) {
  ...
  if (binding == NULL || binding->func == NULL || wglobal.grab_holder == NULL) return;
...
static uint frame_press(WFrame *frame, XButtonEvent *ev, WThing **thing) {
  ...
  if (ev->window == frame->bar_win) {
    ...
    *thing = nth_subthing((WThing *)frame, tabnum);
    ...
    if (cw * 2 > FRAME_W(frame)) cw = frame->w / 2;

    if (ch * 2 > FRAME_H(frame)) ch = FRAME_H(frame) / 2;
...
static uint menu_press(WMenu *menu, XButtonEvent *ev) {
  if (ev->y < menu->title_height) return ACTX_MENUTITLE;
  return ACTX_MENU;
}
...
void handle_button_press(XButtonEvent *ev) {
  ...
  if (!(actx & ACTX_MENU) && wglobal.input_mode == INPUT_CTXMENU) destroy_contextual_menus();

  if (p_clickcnt == 1 && time_in_treshold(ev->time) && p_button == button && p_state == state &&
      wglobal.grab_holder == p_dbltmp && actx == p_actx) {
...
void handle_pointer_motion(XMotionEvent *ev) {
  ...
  if (p_motiontmp == NULL || wglobal.grab_holder == NULL) return;

  fclass = p_motiontmp->func->fclass;
  if (fclass->motion != NULL) {
    fclass->motion(wglobal.grab_holder, ev, dx, dy, p_motiontmp->func, p_motiontmp->arg);
  }
...
static void drag_tab(WFrame *frame, XMotionEvent *ev, int dx, int dy) {
  ...
  if (p_motion == FALSE) {
    if (!WTHING_IS(wglobal.grab_holder, WTHING_CLIENTWIN)) return;

    change_grab_cursor(CURSOR_DRAG);

    XResizeWindow(wglobal.dpy, scr->tabdrag_win, frame->tab_w, frame->bar_h);
    XMapRaised(wglobal.dpy, scr->tabdrag_win);
    ((WClientWin *)wglobal.grab_holder)->flags |= CWIN_DRAG;
    draw_frame_bar(frame, FALSE);
  }

  if (((WClientWin *)wglobal.grab_holder) == NULL) return;
  ...
  if (p_motion == FALSE) draw_tabdrag(((WClientWin *)wglobal.grab_holder));
}
...
static void drag_tab_end(WFrame *frame, XButtonEvent *ev) {
  ...
  if (find_window_at(ev->x_root, ev->y_root, &win)) newframe = find_frame_of(win);
...
void drag_handler(WThing *thing, XMotionEvent *ev, int dx, int dy, WFunction *func, WFuncArg arg) {
  WWinObj *obj;
  int stepsize = 1;
  int moveres_mode = 0;
  ...
  switch (func->opval) {
    ...
    case DRAG_RESIZE:
      ...
      if (moveres_mode & MOVERES_LEFT) dx = -dx;
      if (moveres_mode & MOVERES_TOP) dy = -dy;
...
void drag_end(WThing *thing, XButtonEvent *ev, WFunction *func, WFuncArg arg) {
  ...
  switch (func->opval) {
    ...
    case DRAG_RESIZE_STEPPED:
    case DRAG_RESIZE:
      if (WTHING_IS(obj, WTHING_FRAME)) resize_frame_end((WFrame *)obj);
      break;

    case DRAG_TAB:
      if (WTHING_IS(obj, WTHING_FRAME)) drag_tab_end((WFrame *)obj, ev);
  }
}
