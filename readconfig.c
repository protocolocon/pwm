...
static const char *state_names[] = {"Shift",   "Lock",    "Control", "Mod1",      "Mod2",
                                    "Mod3",    "Mod4",    "Mod5",    "Button1",   "Button2",
                                    "Button3", "Button4", "Button5", "AnyButton", NULL};

static int state_values[] = {ShiftMask, LockMask, ControlMask, Mod1Mask, Mod2Mask,
                             Mod3Mask,  Mod4Mask, Mod5Mask,    Button1,  Button2,
                             Button3,   Button4,  Button5,     AnyButton};
...
static bool get_func(Tokenizer *tokz, Token *toks, int n, WFuncBinder *binder_ret) {
  const char *name = TOK_STRING_VAL(&(toks[0]));

  if (n >= 2) {
    if (!get_arg(tokz, &(toks[1]), binder_ret)) return FALSE;
  }
  ...
  if (binder_ret->func == NULL) {
    warn_obj_line(tokz->name, toks[0].line,
                  ...
                  "Function unknown or invalid argument type.",
                  name);
    return FALSE;
  }
...
static void free_binder(WFuncBinder *binder, bool argstring) {
  if (argstring && binder->arg_type == ARGTYPE_STRING && ARG_TO_STRING(binder->arg) != NULL)
...
static int find_ndx(const char *names[], int num, const char *p) {
  ...
  for (i = 0; (num < 0 || i < num) && names[i] != NULL; i++) {
    if (strcmp(p, names[i]) == 0) return i;
  }
  return -1;
}
...
static bool parse_key(Tokenizer *tokz, Token *tok, uint *mod_ret, int *keysym_ret) {
  ...
  while (*p != '\0') {
    ...
    if (keysym != NoSymbol) {
      if (*keysym_ret != NoSymbol) {
        warn_obj_line(tokz->name, tok->line, "Insane key combination");
        return FALSE;
      }
      *keysym_ret = keysym;
    } else {
      ...
      if (i < 0) {
        warn_obj_line(tokz->name, tok->line, "Unknown key \"%s\"", p);
        return FALSE;
      }
...
static bool do_kbind(Tokenizer *tokz, int n, Token *toks, uint actx) {
  ...
  if (!get_func(tokz, &(toks[2]), n - 2, &binder)) return TRUE; /* just ignore the error */

  if (add_binding(ACT_KEYPRESS, actx, mod, keysym, &binder)) return TRUE;
...
static bool opt_set_mod(Tokenizer *tokz, int n, Token *toks) {
  ...
  if (!parse_key(tokz, &(toks[1]), &mod, &keysym)) return FALSE;
...
static bool opt_mbind_context(Tokenizer *tokz, int n, Token *toks) {
  static char *context_names[] = {"root",    "tab",   "corner", "side",       "window",
                                  "dockwin", "frame", "menu",   "menu_title", NULL};

  static uint context_masks[] = {ACTX_ROOT,    ACTX_TAB,    ACTX_CORNER,
                                 ACTX_SIDE,    ACTX_WINDOW, ACTX_DOCKWIN,
                                 ACTX_C_FRAME, ACTX_MENU,   ACTX_MENUTITLE};

  int i, j;
  const char *p;

  for (i = 1; i < n; i++) {
    if (!TOK_IS_IDENT(&(toks[i]))) goto err;
    p = TOK_IDENT_VAL(&(toks[i]));
    for (j = 0; context_names[j] != NULL; j++) {
      if (strcmp(context_names[j], p) == 0) break;
    }
    if (context_names[j] == NULL) goto err;
...
#define do_mbind_handler(tokz, n, toks, binder) get_func(tokz, &(toks[1]), n - 1, binder);
...
static bool end_mbind(Tokenizer *tokz, int n, Token *toks) {
  ...
  retval = add_pointer_binding(tmp_actx, tmp_state, tmp_button, &tmp_pressb, &tmp_clickb,
                               &tmp_dblclickb, &tmp_motionb);

  if (retval == FALSE) warn_obj_line(tokz->name, tokz->line, "Unable to bind pointer");
...
static bool alloc_entry(WMenuData *mdata) {
  ...
  if (mdata->entries == NULL) {
    ...
    if (mdata->entries == NULL) return FALSE;
  } else {
...
static bool opt_menu_entry(Tokenizer *tokz, int n, Token *toks) {
  ...
  if (!get_func(tokz, &(toks[2]), n - 2, &binder)) goto err;
...
static bool opt_menu_submenu(Tokenizer *tokz, int n, Token *toks) {
  ...
  if (add_submenuent(tmp_menudata, submenu)) return TRUE;
...
static bool opt_screen_font(Tokenizer *tokz, int n, Token *toks) {
  if (tmp_screen == NULL) return TRUE;
...
static bool do_colorgroup(Tokenizer *tokz, Token *toks, WColorGroup *cg) {
  ...
  if (cnt != 4) {
    warn_obj_line(tokz->name, toks[1].line, "Unable to allocate one or more colors");
    return FALSE;
  }
...
static bool opt_screen_workspaces(Tokenizer *tokz, int n, Token *toks) {
  ...
  if (n >= 3) vws = TOK_LONG_VAL(&(toks[2]));
...
static bool opt_screen_dock(Tokenizer *tokz, int n, Token *toks) {
  ...
  for (i = 2; i < n; i++) {
    ...
    for (j = 0; dock_options[j] != NULL; j++) {
      if (strcmp(dock_options[j], p) == 0) break;
    }
    if (dock_options[j] == NULL) goto err;
    flags |= dock_masks[j];
  }
...
static bool begin_winprop(Tokenizer *tokz, int n, Token *toks) {
  ...
  if (winstance != NULL) {
    *winstance++ = '\0';
    if (strcmp(winstance, "*") == 0) winstance = NULL;
  }

  if (strcmp(wclass, "*") == 0) wclass = NULL;
...
static ConfOpt menu_opts[] = {{"entry", "ss?.", opt_menu_entry, NULL},
...
static ConfOpt screen_opts[] = {{"menu_font", "s", opt_screen_menu_font, NULL},
...
static ConfOpt winprop_opts[] = {{"frame", "l", opt_winprop_frame, NULL},
...
static char *includepaths[] = {CF_SYS_CONFIG_LOCATION, NULL};
...
bool read_config(const char *cfgfile) {
  ...
  if (cfgfile == NULL) {
    ...
    if (cfgfile == NULL || access(cfgfile, F_OK) != 0) cfgfile = CF_GLOBAL_CFGFILE;
  }
