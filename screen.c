...
static int my_error_handler(Display *dpy, XErrorEvent *ev) {
  ...
  if (request[0] == '\0') snprintf(request, sizeof(request), "<unknown request>");

  if (ev->minor_code != 0) {
    warn("[%d] %s (%d.%d) %#lx: %s", ev->serial, request, ev->request_code, ev->minor_code,
         ev->resourceid, msg);
  } else {
    warn("[%d] %s (%d) %#lx: %s", ev->serial, request, ev->request_code, ev->resourceid, msg);
  }
#endif
...
static bool do_alloc_color(Display *dpy, Colormap cmap, const char *name, ulong *cret) {
  XColor c;
  bool ret = FALSE;
...
static void scan_initial_windows() {
  ...
  XQueryTree(wglobal.dpy, SCREEN->root, &dummy_root, &dummy_parent, &wins, &nwins);
...
Window create_simple_window(int x, int y, int w, int h, ulong background) {
  return XCreateSimpleWindow(wglobal.dpy, SCREEN->root, x, y, w, h, 0,
                             BlackPixel(wglobal.dpy, SCREEN->xscr), background);
}
...
static void create_wm_windows() {
  ...
  scr->moveres_win =
      create_simple_window(CF_MOVERES_WIN_X, CF_MOVERES_WIN_Y, scr->moveres_win_w,
                           scr->moveres_win_h, GRDATA->tab_sel_colors.pixels[WCG_PIX_BG]);

  /* Create tab drag window */
  scr->tabdrag_win = create_simple_window(0, 0, 1, 1, BlackPixel(wglobal.dpy, scr->xscr));
...
void postinit_screen() {
  ...
  /* font */
  if (grdata->font == NULL) grdata->font = load_font(dpy, CF_FALLBACK_FONT_NAME);

  if (grdata->menu_font == NULL) grdata->menu_font = load_font(dpy, CF_FALLBACK_FONT_NAME);
  ...
  gcvmask = GCForeground | GCBackground | GCLineStyle | GCLineWidth | GCFillStyle | GCJoinStyle |
            GCCapStyle | GCFont;
  ...
  gcv.line_width = 2;
  grdata->copy_gc = XCreateGC(dpy, scr->root, GCLineWidth | GCForeground | GCBackground, &gcv);
