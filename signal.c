...
void check_signals() {
  ...
  if (kill_sig != 0) {
    if (kill_sig == SIGUSR1) {
      if (wglobal.parent == 0) tmp = get_string_property(COMM_WIN, wglobal.atom_private_ipc, NULL);
      wm_restart_other(tmp);
      assert(0);
    }

    if (kill_sig == SIGTERM) wm_exit();
  ...
  if (had_tmr) {
    had_tmr = FALSE;
    if (tmr_handler != NULL) tmr_handler();
    return;
  }
}
...
static void chld_handler(int signal_num) {
  ...
  while ((pid = waitpid(-1, NULL, WNOHANG | WUNTRACED)) > 0) {
    if (wglobal.parent != 0) continue;
    if (wglobal.children == NULL) continue;
...
static void exit_handler(int signal_num) { kill_sig = signal_num; }

static void timer_handler(int signal_num) { had_tmr = TRUE; }
