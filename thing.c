...
void free_thing(WThing *t) {
  if (wglobal.focus_next == t) wglobal.focus_next = NULL /*(WThing*)SCREEN*/;
  if (wglobal.grab_holder == t) wglobal.grab_holder = NULL;
  free(t);
}
...
static WThing *get_next_thing(WThing *first, int filt) {
  while (first != NULL) {
    if (filt == 0 || WTHING_IS(first, filt)) break;
    first = first->t_next;
  };
...
WThing *find_thing(Window win) {
  ...
  if (XFindContext(wglobal.dpy, win, wglobal.win_context, (XPointer *)&thing) != 0) return NULL;
...
WThing *find_thing_t(Window win, int type) {
  ...
  if (thing == NULL || !WTHING_IS(thing, type)) return NULL;

  return thing;
}

WClientWin *find_clientwin(Window win) { return (WClientWin *)find_thing_t(win, WTHING_CLIENTWIN); }
...
WFrame *find_frame_of(Window win) {
  ...
  if (winobj == NULL || !WTHING_IS(winobj, WTHING_FRAME)) return NULL;
...
WWinObj *find_winobj_of(Window win) {
  ...
  if (WTHING_IS(thing, WTHING_FRAME)) return (WWinObj *)thing;
...
WWinObj *winobj_of(WThing *thing) {
  if (WTHING_IS(thing, WTHING_WINOBJ)) return (WWinObj *)thing;

  if (WTHING_IS(thing, WTHING_CLIENTWIN)) return (WWinObj *)CWIN_FRAME((WClientWin *)thing);
