...
static WMenuData winlist_menudata = {"Go to",
                                     "goto_menu",
                                     0,
...
static WMenuData attachlist_menudata = {"Attach",
                                        "attach_menu",
                                        WMENUDATA_CONTEXTUAL,
                                        winlist_menudata_init,
...
static WMenuData detachlist_menudata = {"Detach",
                                        "detach_menu",
                                        0,
                                        winlist_menudata_init,
...
static WMenuData wslist_menudata = {"Go to WS",
                                    "ws_menu",
                                    0,
...
static WMenuData movetows_menudata = {"Move to WS",
                                      "movetows_menu",
                                      WMENUDATA_CONTEXTUAL,
                                      wslist_menudata_init,
...
static void do_free_winlist(WMenuEnt *ents, int n) {
  ...
  for (i = 0; i < n; i++) {
    if (ents[i].name != NULL) free(ents[i].name);
  }
...
static int menuent_cmp(const void *e1, const void *e2) {
  ...
  if (f1->workspace < f2->workspace) return -1;
  if (f1->workspace > f2->workspace) return 1;

  if (f1->frame_id < f2->frame_id) return -1;
  if (f1->frame_id > f2->frame_id) return 1;

  if (ent1->u.winlist.sort < ent2->u.winlist.sort) return -1;
  if (ent1->u.winlist.sort > ent2->u.winlist.sort) return 1;
...
static bool winlist_menudata_init(WMenuData *data, WThing *context) {
  ...
  for (frame = (WFrame *)subthing((WThing *)SCREEN, WTHING_FRAME); frame != NULL;
    ...
    if (data == &attachlist_menudata) {
      if (context != NULL && winobj_of(context) == (WWinObj *)frame) continue;
    }

    for (cwin = first_clientwin((WThing *)frame); cwin != NULL; cwin = next_clientwin(cwin)) {
      if (!CWIN_HAS_FRAME(cwin)) continue;
      ...
      if (ws >= 0) {
        ...
        entryname_length =
            snprintf(entryname + 0, length, "%c%c", (ws == WORKSPACE_STICKY ? '*' : '?'),
                     (cwin == frame->current_cwin ? '+' : '-'));
      }

      snprintf(entryname + entryname_length, length - entryname_length, "  %s", winname);
...
static void winlist_menudata_exec(WMenuEnt *entry, WThing *context) {
  if (entry->u.winlist.clientwin != NULL) goto_clientwin(entry->u.winlist.clientwin);
}
...
static void attachlist_menudata_exec(WMenuEnt *entry, WThing *context) {
  ...
  if (entry->u.winlist.clientwin == NULL) return;

  attachdetach_clientwin((WFrame *)obj, entry->u.winlist.clientwin, FALSE, 0, 0);
}
...
void goto_frame(int id) {
  ...
  if (frame != NULL) {
    ...
    if (frame->current_cwin != NULL) set_focus((WThing *)frame->current_cwin);
  }
}
...
static void movetows_menudata_exec(WMenuEnt *entry, WThing *context) {
  ...
  if (context != NULL) {
    ...
    if (obj != NULL) move_to_workspace(obj, entry->u.num);
  }
}
