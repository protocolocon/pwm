...
static Window lowest_win(WWinObj *obj) {
  if (WTHING_IS(obj, WTHING_FRAME)) return ((WFrame *)obj)->bar_win;

  if (WTHING_IS(obj, WTHING_DOCK)) return ((WDock *)obj)->win;

  if (WTHING_IS(obj, WTHING_MENU)) return ((WMenu *)obj)->menu_win;

  return None;
}

static Window highest_win(WWinObj *obj) {
  if (WTHING_IS(obj, WTHING_FRAME)) return ((WFrame *)obj)->frame_win;
...
static void do_restack_window(Window win, Window other, int stack_mode) {
  ...
  if ((wc.sibling = other) != None) wcmask |= CWSibling;
...
static void restack_windows(WWinObj *obj, Window other, int mode) {
  ...
  if (!WTHING_IS(obj, WTHING_FRAME)) return;
...
bool winobj_is_visible(WWinObj *obj) {
  if (obj->x >= SCREEN->width) return FALSE;

  if (obj->y >= SCREEN->height) return FALSE;

  if (obj->x + obj->w <= 0) return FALSE;

  if (obj->y + obj->h <= 0) return FALSE;
...
static void focus_stack_above(WWinObj *obj) {
  ...
  while (1) {
    ...
    if (!WWINOBJ_IS_MAPPED(obj)) continue;
...
static void do_raiselower_winobj(WWinObj *obj, bool unlink, int raise) {
  ...
  while (tmpobj->stack_above != NULL) tmpobj = tmpobj->stack_above;
  ...
  if (!raise && obj->stack_above != NULL) {
    ...
    for (i = tmpobj->stack_lvl + raise; i < N_STACK_LVLS; ++i) {
      if (screen->winobj_stack_lists[i] != NULL) break;
    }
...
void raise_winobj(WWinObj *obj) { do_raiselower_winobj(obj, TRUE, 1); }

void lower_winobj(WWinObj *obj) { do_raiselower_winobj(obj, TRUE, 0); }

void raiselower_winobj(WWinObj *obj) { do_raiselower_winobj(obj, TRUE, obj->stack_next != NULL); }
...
static void do_restack_winobj(WWinObj *obj, int stack_lvl, WWinObj *stack_above, bool raiselower) {
  WWinObj **listptr;

  /* Unlink the object from stacking list unless it isn't
   * yet stacked (restack_winobj called from add_winobj)
   */
  if (obj->stack_prev != NULL) get_listptr(obj, TRUE);
...
void restack_winobj(WWinObj *obj, int stack_lvl, bool raiselower) {
  if (stack_lvl < 0 || stack_lvl >= N_STACK_LVLS) stack_lvl = LVL_NORMAL;
...
void restack_winobj_above(WWinObj *obj, WWinObj *stack_above, bool raiselower) {
  if (check_loop(obj, stack_above)) {
    warn(
        "Attempt to create loop in winobj stack. "
...
void add_winobj(WWinObj *obj, int ws, int stack_lvl) {
  ...
  if (ws < 0 && ws != WORKSPACE_STICKY) ws = SCREEN->current_workspace;
...
void unlink_winobj_d(WWinObj *obj) {
  ...
  if (wglobal.current_winobj == obj) focus_stack_above(obj);
  ...
  if (wglobal.previous_winobj == obj) wglobal.previous_winobj = NULL;
...
void do_map_winobj(WWinObj *obj) {
  ...
  if (obj->flags & WWINOBJ_HIDDEN) return;
  ...
  if (WTHING_IS(obj, WTHING_MENU)) {
    ...
    if (!WFRAME_IS_SHADE(frame)) XMapWindow(wglobal.dpy, frame->frame_win);

    if (!WFRAME_IS_NO_BAR(frame)) XMapWindow(wglobal.dpy, frame->bar_win);
  }
...
void do_unmap_winobj(WWinObj *obj) {
  ...
  if (!(obj->flags & WWINOBJ_MAPPED)) return;
...
void map_winobj(WWinObj *obj) {
  if (!on_current_workspace(obj)) return;
...
WWinObj *traverse_winobjs(WWinObj *current, WWinObj *root) {
  ...
  if (current->stack_above_list != NULL) return current->stack_above_list;
...
WWinObj *traverse_winobjs_b(WWinObj *p, WWinObj *root, WWinObj **next) {
  ...
  if (p == NULL || *next == NULL || p == root) return NULL;
