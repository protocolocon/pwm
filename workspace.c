...
bool on_current_workspace(WWinObj *obj) { return visible_workspace(obj->workspace); }

int workspace_of(WWinObj *obj) {
  if (!winobj_is_visible(obj)) return WORKSPACE_UNKNOWN;
...
void dodo_switch_workspace(int num) {
  ...
  for (thing = scr->t_children; thing != NULL; thing = next) {
    ...
    if (!WTHING_IS(thing, WTHING_WINOBJ)) continue;

    obj = (WWinObj *)thing;

    if (WWINOBJ_IS_STICKY(obj)) continue;
...
void do_switch_workspace(int num) {
  ...
  if (wglobal.current_winobj == NULL || !on_current_workspace(wglobal.current_winobj)) {
    if (circulate(1) == NULL) {
...
void switch_workspace_num(int num) {
  if (num < 0 || num >= SCREEN->n_workspaces) return;
...
void switch_workspace_hrot(int dir) {
  ...
  if (num < 0) num += scr->workspaces_horiz;
...
static void set_client_workspaces(WWinObj *obj) {
  ...
  while (cwin != NULL) {
    set_integer_property(cwin->client_win, wglobal.atom_workspace_num, obj->workspace);
...
void move_to_workspace(WWinObj *obj, int ws) {
  ...
  if (ws >= SCREEN->n_workspaces) return;
  ...
  while (obj != NULL) {
    ...
    if (visible_workspace(ws)) {
      do_map_winobj(obj);
      if (WTHING_IS(obj, WTHING_FRAME) && (ws == WORKSPACE_STICKY || ows == WORKSPACE_STICKY))
        draw_frame_bar((WFrame *)obj, TRUE);
    } else {
      unmap_winobj(obj);
    }

    if (WTHING_IS(obj, WTHING_FRAME)) set_client_workspaces(obj);
...
void init_workspaces() {
  ...
  XChangeProperty(wglobal.dpy, SCREEN->root, wglobal.atom_workspace_info, XA_INTEGER, 32,
